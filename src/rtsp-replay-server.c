...
static GList *gst_rtsp_media_factory_replay_get_payloaders(
...
static void gst_replay_bin_pad_unblock_and_free(GstReplayBinPad *pad) {
...
/* NOTE: this bin implementation is almost completely taken from
 * rtsp-media-factory-uri but this example doesn't use the
 * GstRTSPMediaFactoryURI object so that we can handle events and messages
...
static void gst_replay_bin_class_init(GstReplayBinClass *klass) {
...
static void gst_replay_bin_init(GstReplayBin *self) {
  ...
  g_signal_connect(self->uridecodebin, "pad-added", G_CALLBACK(pad_added_cb),
                   self);
...
static gboolean send_eos_foreach_srcpad(GstElement *element, GstPad *pad,
                                        gpointer user_data) {
...
static void gst_replay_bin_do_segment_seek(GstElement *element,
                                           GstReplayBin *self) {
  gboolean ret;
  ...
  if (!ret) {
    ...
    gst_element_foreach_src_pad(
        element, (GstElementForeachPadFunc)send_eos_foreach_srcpad, NULL);
  }
}
  ...
  if (GST_MESSAGE_TYPE(message) == GST_MESSAGE_SEGMENT_DONE) {
    ...
    if (next_loop) {
      ...
      gst_element_call_async(
          GST_ELEMENT_CAST(self->uridecodebin),
...
static GstElementFactory *find_payloader(GstReplayBin *self, GstCaps *caps) {
  GList *list;
  ...
  list = gst_element_factory_list_filter(payloaders, caps, GST_PAD_SINK, FALSE);

  if (list == NULL) {
    GList *decoders = gst_rtsp_media_factory_replay_get_decoders(self->factory);
    /* no depayloader, try a decoder, we'll get to a payloader for a decoded
     * video or audio format, worst case. */
    list = gst_element_factory_list_filter(decoders, caps, GST_PAD_SINK, FALSE);
...
replay_bin_sink_probe(GstPad *pad, GstPadProbeInfo *info, GstReplayBin *self) {
  ...
  if (GST_IS_EVENT(GST_PAD_PROBE_INFO_DATA(info))) {
    ...
    case GST_QUERY_SEGMENT:
      ...
      gst_query_set_segment(query, 1.0, GST_FORMAT_TIME, 0,
                            GST_CLOCK_TIME_NONE);
      ret = GST_PAD_PROBE_HANDLED;
      break;
    default:
...
static void gst_replay_bin_do_initial_segment_seek(GstElement *element,
                                                   GstReplayBin *self) {
  gboolean ret;
  GstQuery *query;
  gboolean seekable;
...
static void no_more_pads_cb(GstElement *uribin, GstReplayBin *self) {
  ...
  gst_element_call_async(
      GST_ELEMENT_CAST(self->uridecodebin),
      (GstElementCallAsyncFunc)gst_replay_bin_do_initial_segment_seek, self,
      NULL);
}

static GstElement *gst_replay_bin_new(const gchar *uri, gint64 num_loops,
                                      GstRTSPMediaFactoryReplay *factory,
                                      const gchar *name) {
  GstReplayBin *self;
  ...
  self = GST_REPLAY_BIN(g_object_new(GST_TYPE_REPLAY_BIN, "name", name, NULL));
...
static gboolean payloader_filter(GstPluginFeature *feature, FilterData *self);

#define gst_rtsp_media_factory_replay_parent_class parent_class
G_DEFINE_TYPE(GstRTSPMediaFactoryReplay, gst_rtsp_media_factory_replay,
              GST_TYPE_RTSP_MEDIA_FACTORY);

static void gst_rtsp_media_factory_replay_class_init(
    GstRTSPMediaFactoryReplayClass *klass) {
  GObjectClass *gobject_class = G_OBJECT_CLASS(klass);
  ...
  g_object_class_install_property(
      ...
      g_param_spec_string("uri", "URI", "The URI of the resource to stream",
                          NULL, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));

  g_object_class_install_property(
      gobject_class, PROP_NUM_LOOPS,
...
gst_rtsp_media_factory_replay_init(GstRTSPMediaFactoryReplay *self) {
  ...
  gst_registry_feature_filter(gst_registry_get(),
                              (GstPluginFeatureFilter)payloader_filter, FALSE,
                              &data);
...
static gboolean payloader_filter(GstPluginFeature *feature, FilterData *data) {
  const gchar *klass;
...
gst_rtsp_media_factory_replay_new(const gchar *uri, gint64 num_loops) {
  ...
  factory = GST_RTSP_MEDIA_FACTORY(
      g_object_new(GST_TYPE_RTSP_MEDIA_FACTORY_REPLAY, "uri", uri, "num-loops",
                   num_loops, NULL));
